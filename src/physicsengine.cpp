...
void PhysicsEngine::update(double t, double dt){
  ...
  for(int i = 0 ; i < objects.size() ; i++){
    ...
    for(int j = i + 1 ; j < objects.size() ; j++){
      ...
      if(o1->collide(*o2, contact)){
        nbCol++;
        for(int i = 0 ; i < 100 ; i++){
          for(auto c : contact){
              double j = get_impulse(*o1->state(), *o2->state(), c);
              apply_impulse(*o1->state(), *o2->state(), c, j);
              o1->state()->recalculate();
              o2->state()->recalculate();
          }
        }
      }
    }
  ...
  st->momentum = st->mass * v1;
  st->angularMomentum = st->mass * a1;
}
